...
- 保留三网延迟、地区筛选、彩色标签、地球展示和独立主题身份
- 同步官方 v3.3.4：到期 5 天内标红、6–10 天标黄，无效日期显示 `-`
- 同步官方 v3.3.4：短时指标历史缺少 CPU 时自动回退兼容记录接口
- 同步官方 v3.3.5：详情 Ping 卡片、图例和负载面板指标线遵循后台任务顺序
- 合并三网任务与官方乱序回归夹具，覆盖定制功能和上游修复的组合场景
...
<details open>
<summary><strong>v3.3.12 · 上游修复同步</strong></summary>

- 在不改变 `GlassmorphismThreeNetwork` 独立身份与现有功能的前提下同步官方 v3.3.4–v3.3.5
- 修复到期预警边界、无效到期日期和短时 CPU 历史回退
- Ping 展示统一服从后台公开任务顺序，兼容三网任务选择与旧接口回退
- 合并官方与三网主题的确定性回归覆盖
