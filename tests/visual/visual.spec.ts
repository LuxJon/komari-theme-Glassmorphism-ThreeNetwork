...
test('home card combines CPU core count and load without changing CPU progress semantics', async ({ page }) => {
  ...
  await cpuInfo.focus()
  const tooltip = cpuInfo.getByRole('tooltip')
  await expect(tooltip).toBeVisible()
  await expect(tooltip).toHaveText('CPU 核心数：1 核\n1 分钟负载：0.18\n5 分钟负载：0.14\n15 分钟负载：0.10')
  const tooltipBox = await tooltip.boundingBox()
  const cardBox = await card.boundingBox()
  expect(tooltipBox).not.toBeNull()
  expect(cardBox).not.toBeNull()
  expect(tooltipBox!.x).toBeGreaterThanOrEqual(cardBox!.x)
  expect(tooltipBox!.x + tooltipBox!.width).toBeLessThanOrEqual(cardBox!.x + cardBox!.width)
